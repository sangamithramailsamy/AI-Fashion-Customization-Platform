...
export default function AccountWishlistPage() {
  ...
  if (items.length === 0) {
    return (
      <div>
        <p className="font-body uppercase tracking-[0.3em] text-xs text-muted mb-2">Wishlist</p>
        <h1 className="font-display text-3xl md:text-4xl text-token">Your Wishlist</h1>
        <div className="bg-surface border border-token p-10 text-center mt-8">
          ...
          <h2 className="font-display text-2xl text-token">No saved pieces yet</h2>
  ...
  return (
    <div>
      <p className="font-body uppercase tracking-[0.3em] text-xs text-muted mb-2">Wishlist</p>
      <h1 className="font-display text-3xl md:text-4xl text-token">Your Wishlist</h1>
      <p className="font-body text-sm text-muted mt-2">{items.length} {items.length === 1 ? 'item' : 'items'} saved</p>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  ...
                  <p className="font-body text-xs mt-1" style={inStock ? { color: 'var(--anim-olive)' } : { color: 'var(--text-muted)' }}>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-auto pt-3">
                    ...
                    <Link to={`/product/${p.id}`} className="inline-flex items-center gap-1.5 px-3 py-2 text-xs uppercase tracking-[0.15em] btn-outline">
                      <Eye size={13} /> View
                    </Link>
                    <button
